...
\begin{frame}{A hierarquia que emerge sozinha}
  \framesubtitle{Ninguém programou ``detecte uma borda'' — a rede organizou-se assim durante o treino}

  \begin{center}
    \begin{tikzpicture}[node distance=1.1cm and 1.4cm]
      \node[bloco, minimum height=1.15cm] (baixo) {Baixo nível\\ linhas e bordas};
      \node[bloco, right=of baixo, minimum height=1.15cm] (medio) {Nível médio\\ olhos, nariz, orelhas};
      \node[bloco, right=of medio, minimum height=1.15cm] (alto) {Alto nível\\ estrutura facial};
      \draw[fluxo] (baixo) -- (medio);
      \draw[fluxo] (medio) -- (alto);
    \end{tikzpicture}
  \end{center}

  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Em reconhecimento facial, cada camada aprende um nível de abstração
    \item A hierarquia \emph{emerge} do treinamento — é isso que a palavra
          ``profundo'' compra
  \end{itemize}

  \begin{ideiabox}
    É a versão grande do XOR: lá, a camada oculta construiu ``algum ligado''
    e ``ambos ligados''; aqui, constrói bordas, partes e rostos. O mecanismo
    é o mesmo — muda a escala.
  \end{ideiabox}
\end{frame}
